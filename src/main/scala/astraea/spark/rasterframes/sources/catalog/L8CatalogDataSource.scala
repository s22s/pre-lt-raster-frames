...
package astraea.spark.rasterframes.sources.catalog
...
class L8CatalogDataSource extends DataSourceRegister with RelationProvider {
  ...
  def createRelation(sqlContext: SQLContext, parameters: Map[String, String]): BaseRelation = {
    val path = parameters.getOrElse("path", sceneListFile.toString)
    L8CatalogRelation(sqlContext, path)
  }
}

object L8CatalogDataSource extends LazyLogging {
  val NAME = "landsat8-catalog"
  ...
  private[catalog] def sceneListFile: Path = {
